    ...
    dynamicPage(name: "mainPage") {
        ...
        section { 
            ...
            if (state.showIntro == true ) {
                ...
                paragraph body('<b>Tile Builder</b> allows you to create custom tiles with a broad range of information that can be published to a <b>Hubitat Dashboard</b> using an application completely native to the Hubitat platform. <b>Tile Builder</b> can eliminate the hassle of maintaining a seperate external system just so that you can have an attractive dashboard.')
                part1 = "<ul><li style='font-size:120%'><b>Attribute Monitor</b> tiles can display a single attribute from multiple devices. For example, a tile might show the temperature in 8 different rooms. Tiles are updated whenever a monitored attribute changes.</li>"
                part2 = "<li style='font-size:120%'><b>Activity Monitor</b> tiles can show the level of activity\\inactivity for a group of devices. For example, a tile might list battery devices that have not reported in the last 4 hours as shown below. These tiles are updated on a timed basis.</li>"
                part3 = "<li style='font-size:120%'><b>Coming Soon: Multi Attribute Monitor</b> These tiles will be able to contain attributes from multiple different devices into a single view. Great for displaying related information such as temperature, humidity, lights and motion for a single entity, such as a room.</li>"
                part4 = "<li style='font-size:120%'><b>Tile Builder</b> tiles are endlessly customizeable in an easy to use interface that can generate a very attractive addition to your <b>Hubitat</b> dashboard. For example:</li></ul>"
                paragraph(part1 + part2 + part3 + part4)

                //Get some static CSS\HTML for sample tables
                myHTML = getSample1()
                paragraph '<iframe srcdoc=' + '"' + myHTML + '"' + ' width="380" height="320" style="border:solid" scrolling="no"></iframe>'
                
                myHTML = getSample2()
                paragraph '<iframe srcdoc=' + '"' + myHTML + '"' + ' width="380" height="280" style="border:solid" scrolling="no"></iframe>'
                
                paragraph body('<b>Note</b>: In the current version <b>Tile Builder</b> all tiles are "<u>read-only</u>" with the exception of clicking web links.')
            }
            else {
                input(name: 'btnShowIntro', type: 'button', title: 'Introduction ▶', backgroundColor: 'DodgerBlue', textColor: 'white', submitOnChange: true, width: 2)  //▼ ◀ ▶ ▲
            }
            ...
            //Licensing
            input(name: 'btnShowLicense', type: 'button', title: 'Licensing ▼', backgroundColor: 'DodgerBlue', textColor: 'white', submitOnChange: true, width: 2, newLineBefore: true, newLineAfter: false)  //▼ ◀ ▶ ▲
            myString = "The standard version of <b>Tile Builder</b> is free to use and provides a highly functional addition to the basic Hubitat Dashboard capabilities. "
            myString = myString + "The <b>Advanced</b> version gives access to filtered views plus the Highlights, Styles and Advanced tabs which adds some powerful customizations including icon's, threshold and animations. "
			myString = myString + "See the full <b>Tile Builder</b> documentation at this <a href='https://github.com/GaryMilne/Documentation/blob/main/Tile%20Builder%20Help.pdf'>link.</a></br></br>"
            ...
            //myString += "1) Donate at least <b>\$5</b> via PayPal using this link: <a href='https://www.paypal.com/donate/?business=YEAFRPFHJCTFA&no_recurring=1&item_name=A+donation+of+\$5+or+more+grants+you+license+to+Tile+Builder+Advanced.+Please+leave+your+Hubitat+Community+ID.&currency_code=USD' target=_blank><img src='https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif'></a></br></br>"			
            ...
            if (state.showCreate == true ) {
                ...
                myString = 'This version of <b>Tile Builder</b> has two types of tile available. <b>Activity Monitor</b> tiles allow you to monitor a group of devices for activity\\inactivity using the <b>lastActivityAt</b> attribute. These tiles are refreshed at routine intervals. ' + \
                           '<b>Attribute Monitor</b> allow you to configure a tile that aggregates multiple devices\\single attribute into a single tile. For example, all your room temperatures could be in a single tile. These tiles subscribe to their assigned devices so updates are realtime. Have fun!<br>'
                paragraph note('', myString)
            ...
            if (state.showMore == true ) {
                input(name: 'btnShowMore', type: 'button', title: 'More ▼', backgroundColor: 'DodgerBlue', textColor: 'white', submitOnChange: true, width: 2, newLineBefore: true, newLineAfter: true)  //▼ ◀ ▶ ▲
                label title: bold('Enter a name for this Tile Builder parent instance (optional)'), required: false, width: 3
                ...
                input(name: 'removeLicense'  , type: 'button', title: 'De-Activate Software License', backgroundColor: '#27ae61', textColor: 'white', submitOnChange: true, width: 2)
...
def makeDefaultStyles() {
	...
	styleA = convertStyleStringToMap('#isCustomSize#=false, #tbc#=#ffffff, #tc#=#000000, #isKeyword1#=false, #isKeyword2#=false, #bp#=10, #isHeaders#=false, #htp#=0, #hta#=Center, #ts#=150, #shcolor#=#7a7a7a, #fc#=#000000, #to#=1, #rabc#=#dff8aa, #hbc#=#000000, #shblur#=10, #hts#=100, #bm#=Collapse, #rtc#=#ffffff, #hbo#=1, #iFrameColor#=#fcfcfc, #shver#=2, #tff#=Comic Sans MS, #isTitleShadow#=true, #rtp#=0, #comment#=?, #tp#=3, #th#=Auto, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.3, #isFrame#=false, #shhor#=2, #htc#=#000000, #rbc#=#292929, #tilePreview#=5, #fa#=Center, #rts#=110, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #bo#=1, #fs#=90, #fbc#=#000000, #rta#=Center, #isFooter#=true, #tw#=90, #bfs#=18, #bc#=#ffffff, #ratc#=#000000, #bs#=Solid')
    ...
    styleA = convertStyleStringToMap('#tc#=#050505, #isKeyword1#=false, #bp#=5, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=140, #shcolor#=#d7dce0, #tbc#=#908989, #fc#=#000000, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#b71a3b, #hbc#=#0063b1, #shblur#=2, #hts#=100, #isCustomSize#=false, #bm#=Seperate, #rtc#=#050505, #customWidth#=200, #k1#=?, #hbo#=1, #iFrameColor#=#908989, #shver#=2, #tff#=Arial Black, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=false, #rtp#=0, #top1#=[1], #comment#=?, #tp#=5, #customHeight#=190, #hts2#=125, #th#=85, #tcv1#=100, #isAlternateRows#=false, #isTitle#=true, #br#=25, #ta#=Center, #isComment#=false, #rbo#=0.6, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=2, #htc#=#cbcbc8, #rbc#=#7fb2e7, #top2#=[3], #fa#=Left, #rts#=80, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=80, #fbc#=#000000, #rta#=Center, #isFooter#=false, #ktr1#=?, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#1e1e20, #ratc#=#ffffff, #bw#=3, #bs#=Solid')
	...
	styleA = convertStyleStringToMap('#tc#=#222f3c, #isKeyword1#=false, #bp#=5, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=120, #shcolor#=#ffffff, #tbc#=#aaa9ad, #fc#=#66cbe0, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#dff8aa, #hbc#=#23303e, #shblur#=4, #hts#=100, #isCustomSize#=false, #bm#=Seperate, #rtc#=#d5d5d7, #customWidth#=200, #k1#=?, #hbo#=1, #iFrameColor#=#696969, #shver#=0, #tff#=Arial Black, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=true, #rtp#=0, #top1#=[1], #comment#=?, #tp#=3, #customHeight#=290, #hts2#=125, #th#=Auto, #tcv1#=100, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=0, #htc#=#c0c0c0, #rbc#=#5b6db2, #top2#=[3], #fa#=Center, #rts#=80, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=100, #fbc#=#000000, #rta#=Center, #isFooter#=false, #ktr1#=?, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#ada9a9, #ratc#=#000000, #bw#=2, #bs#=Solid')
    styleB = ['overrides':'#Table#=background: linear-gradient(0deg, #bdc3c7 0%, #2c3e50 40%)']
    style = styleA + styleB
    state.'*Style-AM Blue Grey' = style
	
	styleA = convertStyleStringToMap('#tc#=#fffafa, #isKeyword1#=false, #bp#=3, #ktr2#=?, #isHeaders#=false, #htp#=0, #hta#=Center, #ts#=100, #shcolor#=#7a7a7a, #tbc#=#282828, #fc#=#ffffff, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#dff8aa, #hbc#=#000000, #shblur#=10, #hts#=100, #bm#=Collapse, #rtc#=#ffffff, #k1#=?, #hbo#=1, #iFrameColor#=#282828, #shver#=2, #tff#=Comic Sans MS, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=false, #rtp#=0, #top1#=1, #comment#=?, #tp#=3, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.3, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=2, #htc#=#000000, #rbc#=#292929, #top2#=3, #fa#=Center, #rts#=75, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=75, #fbc#=#000000, #rta#=Center, #isFooter#=true, #ktr1#=?, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#ffffff, #ratc#=#000000, #bw#=2, #bs#=Solid')
    ...
    styleA = convertStyleStringToMap('#isCustomSize#=false, #tbc#=#ffffff, #tc#=#000000, #isKeyword1#=false, #isKeyword2#=false,#bp#=5, #isHeaders#=false, #htp#=0, #hta#=Center, #ts#=150, #shcolor#=#000000, #fc#=#000000, #to#=1, #rabc#=#dff8aa, #hbc#=#ffffff, #shblur#=5, #hts#=100, #bm#=Seperate, #rtc#=#000000, #hbo#=1, #iFrameColor#=#bbbbbb, #shver#=0, #tff#=Roboto, #isTitleShadow#=false, #rtp#=0, #comment#=?, #tp#=5, #th#=Auto, #isAlternateRows#=false, #isTitle#=false, #br#=0, #ta#=Center, #isComment#=false, #rbo#=1, #isFrame#=false, #shhor#=0, #htc#=#000000, #rbc#=#e7e4e4, #tilePreview#=2, #fa#=Center, #rts#=100, #isBorder#=false, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=false, #bo#=1, #fs#=80, #fbc#=#000000, #rta#=Left, #isFooter#=false, #tw#=100, #bfs#=18, #bc#=#050505, #ratc#=#000000, #bw#=2, #bs#=Solid')
    styleB = ['overrides':'?']
    style = styleA + styleB
    state.'*Style-AM Everything Off' = style

    styleA = convertStyleStringToMap('#isCustomSize#=false, #tbc#=#ffffff, #tilePreview#=5, #isFrame#=false, #fbc#=#000000, #isKeyword1#=false, #isKeyword2#=false, #bs#=Solid, #bm#=Collapse, #ft#=%time%, #isHeaders#=true, #htp#=0, #fc#=#000000, #hta#=Center, #ts#=140, #shcolor#=#bfe373, #bc#=#000000, #fs#=80, #rabc#=#E9F5CF, #hbc#=#90c226, #shblur#=4, #hts#=100, #br#=0, #ta#=Center, #rtc#=#000000, #tp#=0, #hbo#=1, #iFrameColor#=#908989, #shver#=0, #tff#=Verdana, #isTitleShadow#=true, #rtp#=0, #comment#=?, #th#=Auto, #isAlternateRows#=true, #bw#=2, #isTitle#=true, #isComment#=false, #fa#=Center, #shhor#=0, #htc#=#000000, #rbc#=#BFE373, #rts#=90, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=false, #tc#=#000000, #rta#=Center, #bp#=10, #isFooter#=true, #tw#=100, #bfs#=18, #ratc#=#000000')
    styleB = ['overrides':'?']
    style = styleA + styleB
    state.'*Style-AM Greens' = style

	styleA = convertStyleStringToMap('#isCustomSize#=false, #tbc#=#ffffff, #tc#=#de5b00, #isKeyword1#=false, #isKeyword2#=false, #bp#=10, #isHeaders#=true, #htp#=10, #hta#=Center, #ts#=200, #shcolor#=#ff1d00, #fc#=#000000, #to#=1, #rabc#=#f69612, #hbc#=#c64a10, #shblur#=2, #hts#=100, #bm#=Seperate, #rtc#=#ffff00, #hbo#=1, #iFrameColor#=#8d8686, #shver#=2, #tff#=Comic Sans MS, #isTitleShadow#=false, #rtp#=6, #comment#=?, #tp#=5, #th#=Auto, #isAlternateRows#=true, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.7, #isFrame#=true, #shhor#=2, #htc#=#000000, #rbc#=#f69612, #tilePreview#=5, #fa#=Center, #rts#=100, #isBorder#=false, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #bo#=0.7, #fs#=80, #fbc#=#000000, #rta#=Center, #isFooter#=false, #tw#=Auto, #bfs#=18, #bc#=#050505, #ratc#=#000000, #bw#=2, #bs#=Solid')
	...
	styleA = convertStyleStringToMap('#isCustomSize#=false, #tc#=#b2e0de, #isKeyword1#=false, #bp#=10, #isHeaders#=false, #htp#=5, #hta#=Center, #ts#=200, #shcolor#=#000000, #tbc#=#ffffff, #fc#=#000000, #hc2#=#CA6F1E, #to#=1, #rabc#=#dff8aa, #hbc#=#9ec1eb, #shblur#=3, #hts#=100, #bm#=Collapse, #rtc#=#282828, #hbo#=1, #iFrameColor#=#282828, #shver#=0, #tff#=Comic Sans MS, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=true, #rtp#=10, #top1#=[1], #comment#=?, #tp#=15, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=1, #k2#=?, #isFrame#=true, #isThreshold2#=false, #shhor#=0, #htc#=#000000, #rbc#=#282828, #top2#=[3], #fa#=Center, #rts#=90, #isBorder#=false, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=60, #fbc#=#282828, #rta#=Center, #isFooter#=false, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#000000, #ratc#=#000000, #bw#=2, #bs#=Solid')
	...
	styleA = convertStyleStringToMap('#tc#=#cfc547, #isKeyword1#=false, #bp#=10, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=200, #shcolor#=#000000, #tbc#=#282828, #fc#=#000000, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#dff8aa, #hbc#=#c74343, #shblur#=3, #hts#=125, #bm#=Seperate, #rtc#=#282828, #k1#=?, #hbo#=0.2, #iFrameColor#=#696969, #shver#=0, #tff#=Comic Sans MS, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=false, #rtp#=10, #top1#=[1], #comment#=?, #tp#=0, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=1, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=0, #htc#=#c5bc44, #rbc#=#696969, #top2#=[3], #fa#=Center, #rts#=90, #isBorder#=false, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=60, #fbc#=#282828, #rta#=Center, #isFooter#=false, #ktr1#=?, #tw#=100, #bfs#=18, #hc1#=#008000, #bc#=#000000, #ratc#=#000000, #bw#=2, #bs#=Solid')
	...
	styleA = convertStyleStringToMap('#isCustomSize#=false, #tc#=#66cbe0, #isKeyword1#=false, #bp#=5, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=150, #shcolor#=#7a7a7a, #tbc#=#ffffff, #fc#=#66cbe0, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#dff8aa, #hbc#=#66cbe0, #shblur#=10, #hts#=140, #bm#=Collapse, #rtc#=#eb822e, #k1#=?, #hbo#=0.8, #iFrameColor#=#696969, #shver#=2, #tff#=Comic Sans MS, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=false, #rtp#=0, #top1#=[1], #comment#=?, #tp#=3, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=1, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=2, #htc#=#eb822e, #rbc#=#d9ddc6, #top2#=[3], #fa#=Center, #rts#=110, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=false, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=100, #fbc#=#000000, #rta#=Center, #isFooter#=false, #ktr1#=?, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#eb822e, #ratc#=#000000, #bw#=2, #bs#=Solid')
    styleB = ['overrides':'?']
    style = styleA + styleB
    state.'*Style-AM Palm Springs' = style
	
	styleA = convertStyleStringToMap('#isCustomSize#=false, #tbc#=#ffffff, #tc#=#000000, #isKeyword1#=false, #isKeyword2#=false, #bp#=10, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=140, #shcolor#=#bfe373, #fc#=#000000, #to#=1, #rabc#=#e9f5cf, #hbc#=#9bdbe8, #shblur#=4, #hts#=100, #bm#=Collapse, #rtc#=#fe7868, #hbo#=0.5, #iFrameColor#=#908989, #shver#=0, #tff#=Verdana, #isTitleShadow#=false, #rtp#=0, #comment#=?, #tp#=0, #th#=Auto, #isAlternateRows#=false, #isTitle#=false, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.5, #isFrame#=false, #shhor#=0, #htc#=#650606, #rbc#=#ffffa0, #tilePreview#=5, #fa#=Center, #rts#=90, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #bo#=1, #fs#=80, #fbc#=#000000, #rta#=Center, #isFooter#=true, #tw#=100, #bfs#=18, #bc#=#000000, #ratc#=#000000, #bw#=2, #bs#=Solid')
	...
	styleA = convertStyleStringToMap('#tc#=#000000, #isKeyword1#=false, #bp#=0, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=120, #shcolor#=#f6cd00, #tbc#=#696969, #fc#=#000000, #hc2#=#ca6f1e, #ttr1#=?, #to#=1, #rabc#=#a8c171, #hbc#=#f9e66c, #shblur#=2, #hts#=85, #isCustomSize#=false, #bm#=Seperate, #rtc#=#000000, #customWidth#=200, #k1#=?, #hbo#=1, #iFrameColor#=#696969, #shver#=2, #tff#=Comic Sans MS, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=false, #rtp#=0, #top1#=[1], #comment#=?, #tp#=0, #customHeight#=190, #hts2#=125, #th#=Auto, #tcv1#=100, #isAlternateRows#=false, #isTitle#=false, #br#=10, #ta#=Center, #isComment#=false, #rbo#=1, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=2, #htc#=#000000, #rbc#=#d1dd2c, #top2#=[3], #fa#=Left, #rts#=70, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=80, #fbc#=#282828, #rta#=Center, #isFooter#=true, #ktr1#=?, #tw#=100, #bfs#=18, #hc1#=#008000, #bc#=#000000, #ratc#=#000000, #bw#=3, #bs#=Dotted')
    styleB = ['overrides':'#Table#=transform: rotate(2deg) translate(0px,8px)']
    style = styleA + styleB
    state.'*Style-AM Tickets' = style
	
	styleA = convertStyleStringToMap('#isCustomSize#=false, #tc#=#412f86, #isKeyword1#=false, #bp#=5, #ktr2#=?, #isHeaders#=true, #htp#=0, #hta#=Center, #ts#=175, #shcolor#=#ffc62f, #tbc#=#e3b994, #fc#=#000000, #hc2#=#ca6f1e, #ttr1#=?, #to#=1, #rabc#=#f7c104, #hbc#=#4c247e, #shblur#=4, #hts#=140, #bm#=Collapse, #rtc#=#f7f7f7, #k1#=open, #hbo#=0.9, #iFrameColor#=#dbc0a9, #shver#=0, #tff#=Brush Script MT, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=true, #rtp#=0, #top1#=[1], #comment#=?, #tp#=3, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.7, #k2#=?, #isFrame#=false, #isThreshold2#=false, #shhor#=0, #htc#=#f7c104, #rbc#=#4d2081, #top2#=[3], #fa#=Center, #rts#=110, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=false, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=0.6, #fs#=90, #fbc#=#000000, #rta#=Center, #isFooter#=false, #ktr1#=[div class=cl99]Open[/div], #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#f7c104, #ratc#=#000000, #bw#=2, #bs#=Solid')
    styleB = ['overrides':'?']
    style = styleA + styleB
    state.'*Style-AM Vikings' = style

    styleA = convertStyleStringToMap('#isCustomSize#=false, #tc#=#d6ae7b, #isKeyword1#=false, #bp#=6, #ktr2#=?, #isHeaders#=false, #htp#=0, #hta#=Center, #ts#=200, #shcolor#=#000000, #tbc#=#ffffff, #fc#=#d6ae7b, #hc2#=#CA6F1E, #ttr1#=?, #to#=1, #rabc#=#dff8aa, #hbc#=#9ec1eb, #shblur#=10, #hts#=100, #bm#=Collapse, #rtc#=#786854, #k1#=?, #hbo#=1, #iFrameColor#=#000000, #shver#=2, #tff#=Brush Script MT, #isThreshold1#=false, #ttr2#=?, #isTitleShadow#=true, #rtp#=0, #top1#=[1], #comment#=?, #tp#=3, #hts2#=125, #th#=Auto, #tcv1#=70, #isAlternateRows#=false, #isTitle#=true, #br#=0, #ta#=Center, #isComment#=false, #rbo#=0.6, #k2#=?, #isFrame#=true, #isThreshold2#=false, #shhor#=2, #htc#=#000000, #rbc#=#ba8c63, #top2#=[3], #fa#=Center, #rts#=150, #isBorder#=true, #isScrubHTML#=true, #rto#=1, #hto#=1, #isOverrides#=true, #tcv2#=30, #hts1#=125, #isKeyword2#=false, #bo#=1, #fs#=90, #fbc#=#560b0b, #rta#=Center, #isFooter#=true, #ktr1#=?, #tw#=90, #bfs#=18, #hc1#=#008000, #bc#=#786854, #ratc#=#000000, #bw#=2, #bs#=Solid')
...
def getSample2() {
    return '<head><style>.cl1{animation:spin 1s linear 0s infinite normal forwards} @keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)} }table.Cd{border-collapse:Collapse;width:90%;margin:Auto;font-family:Comic Sans MS;background-color:#ffffff}.Cd tr{color:rgba(235,130,46,1);text-align:Center}.Cd td{background-color:rgba(217,221,198,1);font-size:110%}.Cd th,.Cd td{border:Solid 2px rgba(235,130,46,1);padding:5px}.Cd th{background:rgba(102,203,224,0.8);color:rgba(235,130,46,1);text-align:Center;font-size:140%}hCd1{color:#008000;font-size:125%}</style></head><body><table class=Cd><tr><th colspan=2>Fan Status</th></tr><tbody><tr><td>Attic</td><td>off</td></tr><tr><td>Bathroom</td><td>off</td></tr><tr><td>Bedroom</td><td><hCd1><div class=cl1>❌</div></hCd1></td></tr><tr><td>Circulation</td><td><hCd1><div class=cl1>❌</div></hCd1></td></tr><tr><td>Porch</td><td>off</td></tr></tbody></table><style>ftCd{display:block;text-align:Center;font-size:90%;color:#66cbe0;</style><ftCd>Wed @ 10:31 AM</ftCd></body>'
}
...
def activateLicense(){
    def UID8 = hubUID.substring(Math.max(hubUID.length() - 8, 0))
    def hub = location.hubs[0]
    String ZID = hub.zigbeeId
    def ZID8 = ZID.substring(Math.max(ZID.length() - 8, 0))
    myResult = obfuscateStrings(UID8.reverse().toString(), ZID8.reverse().toString())
...
def getID(){
    //Get the Quasi Unique Identifier
    String hubUID = getHubUID()
    def UID8 = hubUID.substring(Math.max(hubUID.length() - 8, 0))
    def hub = location.hubs[0]
    String ZID = hub.zigbeeId
    def ZID8 = ZID.substring(Math.max(ZID.length() - 8, 0))
    return ("${UID8.toUpperCase()}-${ZID8.toUpperCase()}")
}
