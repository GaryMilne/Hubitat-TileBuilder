...
metadata {
    ...
    attribute "tile26", "string"    
    attribute "emptyAttribute", "string"   
    attribute "roomsCSS", "string" 
    attribute "alignRoomTilesCSS", "string"
	//Users can select this attribute if they want to just place an Icon somewhere in a Tile Builder Room.
    //This is used to store any custom CSS
    
	//command "test"
    command "initialize"
    command "alignRoomTiles", [ [name:"This command adds some CSS code within the Storage Driver window which causes any TB Rooms data to be positioned 'normally' on the right edge of the screen."]]
    ...
    command "loadRoomsCSS", [ [name:"Here you can load the CSS classes generated by Tile Builder Rooms to allow the Rooms to render fully with the exception of room proportion.*" , type: "STRING" , description: "Paste the CSS from the Tile Builder App here."] ]
...
void test(){
    ...
    //sendEvent(name: "alignRoomTilesCSS", value: "Empty" ) 
}

//Adds some CSS that is used by Tile Builder tiles that repositions them to the right edge of the driver window with a fixed height of 250px.
void alignRoomTiles(){
    myCSS = "<style>.TB{position:relative !important;transform:translateX(-50%) ; height:250px !important}</style><b>Initialized</b>"
    currentValue = alignRoomTilesCSS.toString()
    if (currentValue.contains(myCSS) == false ) sendEvent(name: "alignRoomTilesCSS", value: myCSS ) 
}
...
//Takes the provided CSS which should be copied from Tile Builder App and saves it to the roomsCSS attribute.
//The presence of the Tile Builder CSS allows the tiles to render more accurately within the driver with the exception of room size.
def loadRoomsCSS(roomsCSS){
    mySize = roomsCSS.size()
    if (roomsCSS.size() > 100 ) highlightLabel = "<font color='green'>✔️</font>"
    else highlightLabel = "<font color='red'>❌</font>"
    sendEvent(name: "roomsCSS", value: "<style>" + roomsCSS + "</style>" + highlightLabel ) 
    }

//Creates a tile
void createTile(tileNumber, HTML, description) {
    if (alignRoomTilesCSS == null ) alignRoomTiles()
	...
	if (device.deviceNetworkId == "Tile_Builder_Storage_Device_4") classIdDigit = "D"
	if (device.deviceNetworkId == "Tile_Builder_Storage_Device_5") classIdDigit = "E"
